...
for i = 1:N_sim
    ...
    if (is_valid) % N_data-N+1 times valid, we have to fill the buffer first
        ...
        f_max_eval(cntr) = fit_max_bin_parabola(X_eval, N, ind_f_min, ind_f_max, df);
        ...
        f_w_max_eval(cntr) = fit_max_bin_parabola(X_w_eval, N, ind_f_min, ind_f_max, df);
